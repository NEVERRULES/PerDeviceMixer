...
每次输出设备发生变化（插入耳机、连接蓝牙、断开后回到扬声器，或在程序中手动切换）时，屏幕正下方会出现一个小提示，显示当前输出设备名称和它的主音量；提示持续几秒后自动消失，不会拦截鼠标点击，也不会抢走窗口焦点。连接耳机时提示耳机名称和音量，断开后提示回到的扬声器名称和音量。
...
左耳、右耳或充电盒任一可用电量降至 20% 或以下且没有充电时，托盘会显示一次低电量通知。相同组件持续低电量时不会重复提醒；电量恢复到 20% 以上或耳机断开后才会重新允许提醒。
