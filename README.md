...
> 当前源码版本：`0.4.0-preview.6`；最新公开版本：`0.4.0-preview.1`。项目仍处于预览阶段，建议首次使用时保留 Windows 原生声音设置作为备用入口。
...
- 蓝牙连接期间会合并设备状态风暴并短时重试，一旦蓝牙输出端点变为可用就尽快切过去；
...
```powershell
Get-FileHash .\PerDeviceMixer-0.4.0-preview.6-win-x64-Setup.exe -Algorithm SHA256
```
...
蓝牙耳机开盖或重连时，Windows 可能先发出多次设备状态变化，随后才让实际输出端点进入可用状态。PerDeviceMixer 会把这些通知合并处理，并在短时间内继续检查；只要系统已经报告蓝牙输出端点可用，程序就会立即按最新接入优先级切换。若耳机本身或 Windows 蓝牙栈还没有完成连接，则仍需等待系统把端点变为可用。
