...
- [ ] 6. 加固真实设备事件恢复
  ...
  - [x] 端点消失时使用瞬态容错
  - [x] 新接入输出设备优先与断开回退（连接顺序栈，仅进程内生效）
  - [x] 设备集合通知风暴合并与输出端点短时重试（蓝牙快速切换）
...
- [x] 9. 设备切换提示弹窗（V1 之外的增强，见 `docs/DEVELOPMENT_STATUS.md`）
  - 输出设备变化时在屏幕工作区底部居中短暂显示设备名与主音量
  - 无边框置顶、点击穿透、不抢焦点；2.5 秒自动淡出，事件密集时最长显示 6 秒
  - 设置页可开关（`ShowDeviceSwitchToast`，默认开启）；旧版配置缺少该字段时默认开启，无需迁移
